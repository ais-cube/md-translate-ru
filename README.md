...
Автоматический переводчик документов через Anthropic Claude API.

Принимает **.md, .docx, .pdf** на входе - отдает переведенный документ сразу в **PDF + DOCX + HTML + Markdown** на выходе. Одна команда - весь пайплайн.
...
# Одна команда - все вопросы задаст сам:
python run.py
```

`run.py` покажет интерактивное меню: выбор языка интерфейса (RU/EN), поиск файлов, язык перевода (11 пар), прогноз стоимости, а потом переведет и соберет результат в выбранные форматы. Настройки запоминаются между запусками.
...
**Единый пайплайн** (`run.py`):
- Вход: .md, .docx, .pdf (один файл или папка)
- Выход: PDF + DOCX + HTML + MD (все форматы сразу, в одном файле)
- 11 языковых пар: EN-RU, RU-EN, EN-DE, EN-ES, EN-FR, EN-ZH, EN-JA, EN-PT, DE-EN, FR-EN, ES-EN
- Двуязычный интерфейс (RU/EN) с запоминанием настроек
- Автопроверка зависимостей при запуске
- Обнаружение и диалог по картинкам в документах
- Атрибуция "Переведено с помощью md-translate-ru" на языке перевода
- Дедупликация заголовков в выходе (защита от артефактов LLM)
- Прогноз стоимости и времени перед запуском
- Бюджетный контроль, Ctrl+C для остановки
- Папка `output/` по умолчанию для результатов
...
**Генерация PDF + HTML** (`convert.py`):
- PDF с кириллицей через локальные шрифты DejaVu Sans (без системных зависимостей)
- HTML с адаптивным дизайном и print-friendly стилями
- Поддержка заголовков, таблиц, code blocks, списков, цитат
- Работает на любой ОС - все шрифты в папке `fonts/`
...
my-project/
  ...
  convert.py                # генерация PDF + HTML
  fonts/                    # локальные шрифты (DejaVu, Liberation)
...
### 6. Генерация PDF + HTML

После перевода - конвертация в финальные форматы:

```bash
# Все форматы (PDF + HTML + MD)
python convert.py
...
# Указать входную папку (по умолчанию docs_ru)
python convert.py --input my_translations
```

Шрифты DejaVu Sans с полной кириллицей уже в папке `fonts/` - ничего устанавливать не нужно.
...
| Флаг | Описание |
|---|---|
| `--file FILENAME` | Конвертировать конкретный файл |
| `--format FORMAT` | `pdf`, `html`, `md` или `all` (по умолчанию) |
| `--input DIR` | Входная папка (по умолчанию: `docs_ru`) |
| `--output-dir DIR` | Выходная папка (по умолчанию: `output`) |
...
| Проблема | Решение |
|---|---|
| `ERROR: Missing packages:` | `pip install -r requirements.txt` |
...
| PDF без кириллицы | Проверить наличие шрифтов в `fonts/` |
| `Папка docs_ru не найдена` | Сначала запустить `translate_api.py` |
| Сменить язык интерфейса | Удалить `.run_config.json` или нажать `s` при запуске |

## Структура репозитория

```
md-translate-ru/
  run.py                        # единый пайплайн: вход -> перевод -> выход
  translate_api.py              # перевод текста Markdown (детальный)
  translate_images.py           # перевод изображений (Vision API)
  convert.py                    # генерация PDF + HTML (standalone)
  requirements.txt              # зависимости
  .run_config.json              # настройки интерфейса (создается автоматически)
  output/                       # выходные файлы (по умолчанию)
  LICENSE                       # MIT
  CHANGELOG.md
  CONTRIBUTING.md
  fonts/                        # локальные шрифты с кириллицей
    DejaVuSans.ttf
    DejaVuSans-Bold.ttf
...
- Python 3.10+
- Пакеты: `anthropic`, `rich`, `fpdf2`, `markdown`, `python-docx`, `pdfplumber` (все в `requirements.txt`)
- API-ключ Anthropic с балансом
- При запуске `run.py` проверит зависимости и подскажет, что установить
